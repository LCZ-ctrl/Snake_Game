...
int main() {
    ...
    // 开场动画：启动时直接显示 Logo
    enum IntroState introState = INTRO_FINISHED;
    float logoStartTime = GetTime();
    const float logoDuration = 3.5f;
    const float logoPause = 0.8f;

    // 启动音频线程：负责 UpdateMusicStream 与自动切歌（但仅当 g_musicStarted==true 且未暂停时）
    ...
    while (!WindowShouldClose() && !shouldExit) {
        ...
        // 更新开场动画状态（Logo -> 完成）
        if (introState == LOGO_ANIMATION) {
            float elapsedLogo = currentFrameTime - logoStartTime;
            if (elapsedLogo >= (logoDuration + logoPause)) {
                introState = INTRO_FINISHED;
                // 一旦开场动画结束，若当前处于主菜单则开始播放音乐
                if (game.menuActive) {
                    std::lock_guard<std::mutex> lk(g_audioMutex);
        ...
        // 如果进入主菜单（开场已结束且 menuActive 为 true），确保音乐已启动
        if (introState == INTRO_FINISHED && game.menuActive && !g_musicStarted.load()) {
            std::lock_guard<std::mutex> lk(g_audioMutex);
            if (!game.playlist.empty()) {
                PlayMusicStream(game.playlist[game.currentMusicIndex]);
                if (game.currentMusicIndex == 0) SetMusicVolume(game.playlist[0], game.musicVolume1);
        ...
        // 游戏逻辑（开场动画结束后）
        if (introState == INTRO_FINISHED) {
            game.UpdateScreenShake(deltaTime);

            // 音乐控制（所有音频 API 访问需要加锁）
            if (IsKeyPressed(KEY_SPACE)) {
                std::lock_guard<std::mutex> lk(g_audioMutex);
                if (!g_musicStarted.load()) {
                    // 未启动音乐：若按空格并且 playlist 非空，则开始播放
                    if (!game.playlist.empty()) {
                        PlayMusicStream(game.playlist[game.currentMusicIndex]);
                        if (game.currentMusicIndex == 0) SetMusicVolume(game.playlist[0], game.musicVolume1);
                        else if (game.currentMusicIndex == 1) SetMusicVolume(game.playlist[1], game.musicVolume2);
            ...
            if (game.mode == 2) {
                ...
                const float minInterval = 0.04f; // 下限
        ...
        if (introState == LOGO_ANIMATION) {
            ...
            float elapsedLogo = currentFrameTime - logoStartTime;
            DrawLogoAnimation(elapsedLogo, GetScreenWidth(), GetScreenHeight());
        }
        else {
            // 正常游戏界面背景
